...
class blk(gr.sync_block):  
    """Angle of Arrival Estimation for a 3-element ULA"""

    def __init__(self, frequency=2.4e9, array_spacing=0.5):  
        """Initialize parameters"""
        gr.sync_block.__init__(
            self,
            name='Angle of Arrival Estimation (3-Element ULA)',
            in_sig=[np.float32, np.float32, np.float32],  # Three phase inputs (already in radians)
    ...
    def work(self, input_items, output_items):
        ...
        for i in range(num_samples):
            ...
            delta_phi_2 = np.angle(np.exp(1j * (phi_2[i] - phi_1[i])))  # Between Antenna 1 & 2
            
            # Take the average phase difference for robustness
            delta_phi_avg = (delta_phi_1 + delta_phi_2) / 2  
